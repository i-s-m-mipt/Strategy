...
class Connector(Spot):
    ...
    def get_current_state(self, symbol: str):
        ...
        netAsset, free, borrowed, locked = \
            tuple(map(float, (data['netAsset'], data['free'], data['borrowed'], data['locked'])))
        if abs(netAsset) < precision:
            return 'N'
        elif netAsset > 0.0 and \
                free * Connector.prices[symbol] > 10.:
            return 'L'
        elif netAsset < 0.0 and \
                borrowed * Connector.prices[symbol] > 10.:
            return 'S'
        else:
            return 'N'
